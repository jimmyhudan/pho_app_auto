...
import com.pho.hudan.util.DateUtil;
import com.pho.hudan.util.ScreenshotUtil;
...
public class TestListener extends TestListenerAdapter {
	...
	public void onTestFailure(ITestResult tr) {
		...
		String screenshotDir = surefireDir + File.separator + "screenshot"  + File.separator
				+ dateStr;
		// 5：截图的功能--》可能有一大堆代码
		File screenshotFile =ScreenshotUtil.takeScreenshot(appbase.getAndroidDriver(), screenshotDir);
		...
		String newStr = "http://localhost:80/";//硬编码
		...
		//在每一次测试用例执行失败的时候把retryCount置为1,解决重试机制和dataprovider的冲突问题
	...
	public void onTestSuccess(ITestResult tr) {
		...
		//在每一次测试用例执行成功的时候把retryCount置为1,解决重试机制和dataprovider的冲突问题
		TestngRetry.setRetryCount(1);
